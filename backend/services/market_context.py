...
# ── Trend momentum fetcher (extracted to services/trend_context.py) ───────────

from services.trend_context import (  # noqa: E402
    TREND_CACHE_FILE,
    fetch_trend_context,
    get_trend_freshness_note,
    build_trend_block as _build_trend_block,
    track_trend_health,
)
...
async def fetch_market_context(ticker: str, event_keywords: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    if iron.get("status") not in (STALE, None) and _validate_iron_ore(iron.get("price", 0), "post-fetch"):
        try:
            ...
            logger.debug("Redis LKG cache write failed (best-effort): %s", e)
