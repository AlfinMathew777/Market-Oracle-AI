...
const ChokepointRiskPanel = ({ onSimulateChokepoint }) => {
  ...
  const fetchChokepoints = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/data/chokepoints?enriched=false`);
  ...
  const sorted = Object.values(data.chokepoints).sort((a, b) => b.risk_score - a.risk_score);
  const supplyAtRisk = data.global_supply_at_risk_pct || 0;
  const riskColor = supplyAtRisk > 25 ? '#ff2222' : supplyAtRisk > 10 ? '#ff8800' : '#44ff88';
  ...
  return (
    <div style={styles.panel}>
      ...
      <div style={styles.header}>
        <span style={styles.title}>⚓ CHOKEPOINT RISK</span>
        <span style={{ ...styles.supplyBadge, color: riskColor, borderColor: riskColor }}>
          {supplyAtRisk}% SUPPLY AT RISK
        </span>
      </div>

      <div style={styles.meta}>
        <span style={{ color: '#ff4444' }}>● {data.critical_count} CRITICAL</span>
        ...
        {sorted.map((cp) => (
          <div key={cp.chokepoint_id} style={styles.row}>
            ...
            <div
              ...
              onClick={() => setExpanded(expanded === cp.chokepoint_id ? null : cp.chokepoint_id)}
            >
              <div style={styles.rowLeft}>
                <span style={{ color: cp.color, fontSize: '10px', marginRight: '5px' }}>●</span>
                <span style={styles.cpName}>{cp.name}</span>
              </div>
              <div style={styles.rowRight}>
                <span style={styles.flowText}>{cp.oil_flow_mbd}mb/d</span>
                <span style={{ ...styles.pctText, color: cp.color }}>{cp.pct_global_supply}%</span>
              </div>
            </div>
            ...
            {expanded === cp.chokepoint_id && (
              <div style={styles.expanded}>
                ...
                {cp.asx_tickers_affected && cp.asx_tickers_affected.length > 0 && (
